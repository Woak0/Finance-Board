...
class ApiKeyDialog(QDialog):
    def __init__(self,parent=None):super().__init__(parent);self.setWindowTitle("AI Feature Setup");self.setMinimumWidth(400);self.api_key=None;layout=QVBoxLayout(self);info_label=QLabel("To enable AI features, a free API key from OpenRouter.ai is required...");info_label.setWordWrap(True);link_label=QLabel('<a href="https://openrouter.ai/keys" style="color:#88c0d0;">Open OpenRouter.ai Keys Page</a>');link_label.setOpenExternalLinks(True);self.key_input=QLineEdit();self.key_input.setPlaceholderText("Paste your API key here (e.g., sk-or-...)");button_box=QDialogButtonBox(QDialogButtonBox.StandardButton.Ok|QDialogButtonBox.StandardButton.Cancel);button_box.accepted.connect(self.accept);button_box.rejected.connect(self.reject);layout.addWidget(info_label);layout.addWidget(link_label);layout.addWidget(self.key_input);layout.addWidget(button_box)
    def accept(self):
...
class MainWindow(QMainWindow):
    def __init__(self, **managers):
        ...
        self.check_and_request_api_key()
    ...
    def create_menu_bar(self):
        menu_bar=self.menuBar();s=self.style();file_menu=menu_bar.addMenu("File");tools_menu=menu_bar.addMenu("Tools");ai_menu=menu_bar.addMenu("AI Tools");save_action=QAction(QIcon(s.standardIcon(QStyle.StandardPixmap.SP_DialogSaveButton)),"Save",self);save_action.setShortcut(QKeySequence.StandardKey.Save);save_action.triggered.connect(self.save_and_refresh);file_menu.addAction(save_action);export_action=QAction(QIcon(s.standardIcon(QStyle.StandardPixmap.SP_ArrowUp)),"Export All to CSV",self);export_action.triggered.connect(self.export_all_data);file_menu.addAction(export_action);clear_action=QAction(QIcon(s.standardIcon(QStyle.StandardPixmap.SP_TrashIcon)),"Clear All Data...",self);clear_action.triggered.connect(self.clear_all_data);file_menu.addAction(clear_action);snowball_action=QAction(QIcon(s.standardIcon(QStyle.StandardPixmap.SP_MediaSeekForward)),"Debt Payoff Strategy",self);snowball_action.triggered.connect(self.show_debt_snowball);tools_menu.addAction(snowball_action);whatif_action=QAction(QIcon(s.standardIcon(QStyle.StandardPixmap.SP_FileDialogDetailedView)),"What-If Calculator",self);whatif_action.triggered.connect(self.show_what_if_calc);tools_menu.addAction(whatif_action);networth_action=QAction(QIcon(s.standardIcon(QStyle.StandardPixmap.SP_ToolBarVerticalExtensionButton)),"Log Net Worth Snapshot",self);networth_action.triggered.connect(self.log_net_worth);tools_menu.addAction(networth_action);health_check_action=QAction(QIcon(s.standardIcon(QStyle.StandardPixmap.SP_ComputerIcon)),"Get Financial Health Check",self);health_check_action.triggered.connect(self.run_ai_health_check);ai_menu.addAction(health_check_action);chat_action=QAction(QIcon(s.standardIcon(QStyle.StandardPixmap.SP_MessageBoxInformation)),"Start AI Chat",self);chat_action.triggered.connect(self.run_ai_chat);ai_menu.addAction(chat_action);command_bar_action=QAction(QIcon(s.standardIcon(QStyle.StandardPixmap.SP_CommandLink)),"Use AI Command Bar",self);command_bar_action.triggered.connect(self.run_ai_command_bar);ai_menu.addAction(command_bar_action)

    def create_dashboard_tab(self):
        widget=QWidget();main_layout=QVBoxLayout(widget);title=QLabel("Finance Dashboard");title.setObjectName("Header");main_layout.addWidget(title,alignment=Qt.AlignmentFlag.AlignHCenter);top_layout=QHBoxLayout();summary_container=QWidget();summary_layout=QFormLayout(summary_container);self.summary_labels={'debt_incurred':QLabel(),'debt_paid':QLabel(),'debt_remaining':QLabel(),'debt_eta':QLabel(),'loan_out':QLabel(),'loan_repaid':QLabel(),'loan_remaining':QLabel(),'net_position':QLabel()};[l.setFont(QFont("Segoe UI",11))for l in self.summary_labels.values()];self.summary_labels['net_position'].setFont(QFont("Segoe UI",12,QFont.Weight.Bold));summary_layout.addRow(QLabel());summary_layout.addRow(QLabel("<b>DEBTS (Money You Owe)</b>"));summary_layout.addRow("Total Debt Incurred:",self.summary_labels['debt_incurred']);summary_layout.addRow("Total Payments Made:",self.summary_labels['debt_paid']);summary_layout.addRow("<b>Remaining Debt Balance:</b>",self.summary_labels['debt_remaining']);summary_layout.addRow("<i>Est. Debt-Free Date:</i>",self.summary_labels['debt_eta']);summary_layout.addRow(QLabel());summary_layout.addRow(QLabel("<b>LOANS (Money Owed to You)</b>"));summary_layout.addRow("Total Loaned Out:",self.summary_labels['loan_out']);summary_layout.addRow("Total Repaid to You:",self.summary_labels['loan_repaid']);summary_layout.addRow("<b>Remaining to Collect:</b>",self.summary_labels['loan_remaining']);summary_layout.addRow(QLabel());summary_layout.addRow(QLabel("---"));summary_layout.addRow("<h2>Net Financial Position:</h2>",self.summary_labels['net_position']);top_layout.addWidget(summary_container,1);self.pie_chart_canvas=self.create_pie_chart();top_layout.addWidget(self.pie_chart_canvas,1);main_layout.addLayout(top_layout);self.line_chart_canvas=self.create_line_chart();main_layout.addWidget(self.line_chart_canvas,1);return widget
    ...
    def update_entry_status(self,entry:LedgerEntry):
        ...
        if balance<=0 and entry.status=='active':entry.status='paid'
        elif balance>0 and entry.status=='paid':entry.status='active'
    ...
    def check_and_request_api_key(self):
        if not self.config.get("OPENROUTER_API_KEY"):
            ...
            if dialog.exec():self.config["OPENROUTER_API_KEY"]=dialog.api_key;save_config(self.config);self.ai_analyser.api_key=dialog.api_key;QMessageBox.information(self,"Success","API Key saved!")
            else:QMessageBox.warning(self,"AI Disabled","No API key provided.")